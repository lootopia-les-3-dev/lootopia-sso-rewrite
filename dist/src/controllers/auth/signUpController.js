...
export const signUpController = async (c) => {
    ...
    if (existingUser) {
        await sendEmail(email, "Tentative de création de compte Lootopia", `Un compte existe déjà avec cette adresse. Si vous souhaitez vous connecter, rendez-vous sur ${process.env.BASE_URL}/auth/signin.`);

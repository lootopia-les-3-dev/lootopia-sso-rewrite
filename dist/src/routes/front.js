...
Front.get("/", (c) => {
    ...
    return c.html(_jsx(AuthPage, { email: email, callbackUrl: callbackUrl }));
});
Front.get("/signin", (c) => {
    const email = c.req.query("email");
    const callbackUrl = c.req.query("callback_url");
...
Front.get("/verify", (c) => {
    return c.html(_jsx(VerifyPage, {}));
});
Front.get("/complete", (c) => {
    const callbackUrl = c.req.query("callback_url");
    return c.html(_jsx(CompletePage, { callbackUrl: callbackUrl }));
});
Front.get("/success", (c) => {
    return c.html(_jsx(SuccessPage, {}));
});
